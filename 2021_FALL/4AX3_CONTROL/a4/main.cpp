...
void Satellite::Print(FILE *file) { fprintf(file, "%f\t%f\t", px, py); }
...
class Robot {
  public:
    ...
    void jacobianF(Eigen::MatrixXd &F, const Eigen::VectorXd &X);
    void jacobianV(Eigen::MatrixXd &V, const Eigen::VectorXd &X);
    void jacobianH(Eigen::MatrixXd &H, const Eigen::VectorXd &X, double px, double py);

    // HERE
    // THE NEW JACOBIAN YOU HAVE TO IMPLEMENT,D Distance
    void jacobianHD(Eigen::MatrixXd &H, const Eigen::VectorXd &X, double px, double py);
  ...
  private:
    ...
    Eigen::MatrixXd JH; // jacobian of H wiht respect to distance and angle
...
void Robot::jacobianF(Eigen::MatrixXd &F, const Eigen::VectorXd &X) {
    F << 1, 0, -dt * speed * sin(X(2)), 0, 1, dt * speed * cos(X(2)), 0, 0, 1;
}

// THis is df/dv and df/dsteer

void Robot::jacobianV(Eigen::MatrixXd &V, const Eigen::VectorXd &X) {
...
void Robot::jacobianH(Eigen::MatrixXd &H, const Eigen::VectorXd &X, double px, double py) {
    double var = (px - X(0)) * (px - X(0)) + (py - X(1)) * (py - X(1));
    const double dist = sqrt(var);

    H << -(px - X(0)) / dist, -(py - X(1)) / dist, 0, (py - X(1)) / var, -(px - X(0)) / var, -1;
}
...
void Robot::LocalizeC(void) {
    ...
    Eigen::MatrixXd K(1, 3); // This is actually a 1x3
...
void Robot::LocalizeM(void) {
    ...
    K = Eigen::MatrixXd(2, 3); // This is a 2x3
...
// Observation noise
        ...
        jacobianH(JH, X, markers[i].px, markers[i].py);
